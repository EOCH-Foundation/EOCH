...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // if sub-cent change is required, the fee must be raised to at least GetMinTxFee()
                // or until nChange becomes zero
                // NOTE: this depends on the exact behaviour of GetMinFee
                if (nFeeRet < GetMinTxFee() && nChange > 0 && nChange < CENT)
                {
                    int64_t nMoveToFee = min(nChange, GetMinTxFee() - nFeeRet);
                    nChange -= nMoveToFee;
                    nFeeRet += nMoveToFee;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-EOCH-address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CEOCHAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CEOCHAddress(address).ToString());
}
